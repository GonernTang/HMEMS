...
class VecMem:
    def __init__(self, config: VecMemConfig):
        ...
        embed_config = BaseEmbedderConfig(
            model=os.getenv("MODEL0", "text-embedding-3-small"),
            api_key=os.getenv("OPENAI_API_KEY"),
            openai_base_url=os.getenv("OPENAI_BASE_URL"),
        )
        self.embedder = OpenAIEmbedding(embed_config)
        self.vec_store = FlatIndex(embedding_dim=1024)
        ...
        self.openai_client1 = OpenAI(base_url=os.getenv("M1_BASE_URL"))
        self.openai_client2 = OpenAI(base_url=os.getenv("M2_BASE_URL"))
        ...
        if self.enable_semantic_memory:
            self.semantic_memory = SemanticMemory(embedding_dim=1024)
    ...
    def save(self, dir_path: str):
        """Save all memory states to disk."""
        import os
        os.makedirs(dir_path, exist_ok=True)
        # Save vec_mem payload mapping
        with open(os.path.join(dir_path, "vec_mem_payload_mapping.json"), "w") as f:
            json.dump(self._payload_mapping, f, indent=2, default=str)
        # Save aug_mem (episodic memories)
        self.aug_mem.save(dir_path)
        # Save semantic memory if enabled
        if self.enable_semantic_memory:
            self.semantic_memory.save(dir_path)
        logger.info(f"VecMem saved to {dir_path}")

    def load(self, dir_path: str):
        """Load all memory states from disk."""
        import os
        # Load vec_mem payload mapping
        payload_file = os.path.join(dir_path, "vec_mem_payload_mapping.json")
        if os.path.exists(payload_file):
            with open(payload_file, "r") as f:
                self._payload_mapping = json.load(f)
            # Update id_assigner
            if self._payload_mapping:
                self.id_assigner = max(int(k) for k in self._payload_mapping.keys()) + 1
        # Load aug_mem (episodic memories)
        self.aug_mem.load(dir_path)
        # Load semantic memory if enabled
        if self.enable_semantic_memory:
            self.semantic_memory.load(dir_path)
        logger.info(f"VecMem loaded from {dir_path}")
