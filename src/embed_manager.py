...
from conv_loader import Conversation, Question
import numpy as np
import os
...
class EmbedManager:
    def __init__(self):
        ...
        self.embedder = OpenAIEmbedding(config)
        ...
        for session in conversation.sessions:
            ...
            for i in range(0, len(session.messages), 2):
                ...
                if i + 1 < len(session.messages):
                    ...
                    msg = (
                        f"[{message_a.speaker}]: {message_a.content} "
                        f"[{message_b.speaker}]: {message_b.content} "
                ...
                embedding = self.embedder.embed(msg)
