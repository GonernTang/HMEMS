...
class NaiveAugMem:
    def __init__(self, aug_config: AugConfig):
        self.vec_store: NaiveStore = NaiveStore(embedding_dim=1024)
        self.openai_client = OpenAI(
            base_url=os.getenv("M1_BASE_URL"), api_key=os.getenv("OPENAI_API_KEY")
        )
        ...
        self.embeder = OpenAIEmbedding(embed_config)
        self.id_counter = 0
    ...
    def try_merge_new_memory(
        ...
        if should_merge == "yes":
            ...
            if not new_mem:
                logger.warning("Merge decision is 'yes' but no merged_memory provided")
                return False, ""
    ...
    def save(self, dir_path: str):
        ...
        # Save raw_store (EpisodicNote objects)
        episodic_data = {
            str(eid): {
                "raw_source": note.raw_source,
                "raw_source_ids": note.raw_source_ids,
            }
            for eid, note in self.raw_store.items()
        }
        with open(os.path.join(dir_path, "episodic_memories.json"), "w") as f:
            json.dump(episodic_data, f, indent=2)
        # Save vec_store (vector store with payloads)
        self.vec_store.save(dir_path)
        logger.info(f"Episodic memories saved to {dir_path}")

    def load(self, dir_path: str):
        """Load episodic memories from disk."""
        import os
        episodic_file = os.path.join(dir_path, "episodic_memories.json")
        if os.path.exists(episodic_file):
            with open(episodic_file, "r") as f:
                episodic_data = json.load(f)
            self.reset()
            for eid_str, data in episodic_data.items():
                eid = int(eid_str)
                self.raw_store[eid] = EpisodicNote(
                    data["raw_source"], data["raw_source_ids"]
                )
                if eid >= self.id_counter:
                    self.id_counter = eid + 1
        # Load vec_store
        self.vec_store.load(dir_path)
        logger.info(f"Episodic memories loaded from {dir_path}")